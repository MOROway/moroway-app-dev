...
if [[ ! -e "$fontFileOut" ]] || [[ "$fontFileOut" -ot "$fontFile" ]]; then
	fonttools varLib.instancer "$fontFile" FILL=1 GRAD=125 opsz=48 wght=450 -o "$fontFileOut" || exit 11
fi
...
if [[ ! -e "$fontFileOut" ]] || [[ "$fontFileOut" -ot "$fontFile" ]]; then
	fonttools varLib.instancer "$fontFile" wdth=480 wght=500 -o "$fontFileOut" || exit 12
fi
fontFileOut="$fontsOutDir/$fontPath/Roboto-Regular.ttf"
if [[ ! -e "$fontFileOut" ]] || [[ "$fontFileOut" -ot "$fontFile" ]]; then
	fonttools varLib.instancer "$fontFile" wdth=480 wght=400 -o "$fontFileOut" || exit 13
fi
cp -p "$fontBasePath/$fontPath/"*.{txt,css} "$fontsOutDir/$fontPath/"
